...
impl<'a, S: ContextProvider> SqlToRel<'a, S> {
    pub(super) fn sql_function_to_expr(
        ...
        if self.schema_provider.options().sql_parser.prioritize_udf {
            // user-defined functions (UDF)
            if let Some(udf) = self.try_sql_fn_to_udf(&name, &function, schema, planner_context)? {
                return Ok(udf);
            }
        }
    ...
    fn try_sql_fn_to_udf(
        ...
        if let Some(fm) = self.schema_provider.get_function_meta(&name) {
            let args =
                self.function_args_to_expr(function.args.clone(), schema, planner_context)?;
