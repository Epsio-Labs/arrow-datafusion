...
use arrow::datatypes::{
    DataType, Field, FieldRef, Fields, TimeUnit, DECIMAL128_MAX_PRECISION, DECIMAL128_MAX_SCALE,
...
fn signature(lhs: &DataType, op: &Operator, rhs: &DataType) -> Result<Signature> {
    ...
    match op {
        ...
        AtArrow if lhs == &json_type() && rhs == &json_type() => {
        ...
        ArrowAccess | LongArrow => Ok(Signature {
            lhs: lhs.clone(),
            rhs: rhs.clone(),
